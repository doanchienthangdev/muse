...
/muse:who should I rewrite this service in Rust?
  → Scores all 25 personas. Top 3: Elon Musk 72/80 (the algorithm +
...
Muse is a **persona runtime**. It ships with a curated starter pack of 25 great mentors and a full set of commands to use them, compose them, build new ones, and measure them. The starter pack is not a ceiling — it is a baseline. You can add personas locally via `/muse:build` in ~15 interactive questions, or pull new personas from the community via a future persona catalog (roadmap item — see § Roadmap).

Shipped artifacts:

### 🎭 Starter pack — 25 cognitive personas
...
You don't need to memorize 25 personas (or 80 someday) to use muse. Three entry points, from cold-start to expert:
...
MUSE TRIAGE — scoring 25 personas against your question
...
## Commands reference (35+ total — v2.29.0-alpha)
...
### 🎭 Persona sessions — the starter pack (25)
...
| Command | Attrs | What it does |
|---|---|---|
| `/muse:benchmark [--baseline \| --diff \| --quick \| --persona=<id>]` | 💾 | Measure persona distinctiveness + voice discipline + schema compliance + mode fit. 4 categories, 11 measures. Grade A maintained across all 25 personas. |
...
8. **Extensible by design.** The starter pack is 25 personas (growing). The runtime supports arbitrary local personas via `/muse:build`. A future persona catalog (§ Roadmap) will let users browse and clone individual personas without cloning the whole framework.
...
- `/muse:benchmark` measures distinctiveness empirically, not just by taste. Grade A across all 25 personas.
...
**Shipping today** (v2.29.0-alpha):
- 25 starter-pack personas across 8+ categories (philosophy, first-principles, design, systems, strategy, execution, investing, management/culture, AI-industry)
- 35+ slash commands (13 meta + 25 persona), 5 adaptive session modes (QUICK, STANDARD, DEEP, CRITIC, EXPLORE)
...
v2.29 is the mature foundation; v3.0 is when the catalog becomes real.
