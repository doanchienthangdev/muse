...
/muse:who should I rewrite this service in Rust?
  → Scores all 24 personas. Top 3: Elon Musk 72/80 (the algorithm +
...
Muse is a **persona runtime**. It ships with a curated starter pack of 24 great mentors and a full set of commands to use them, compose them, build new ones, and measure them. The starter pack is not a ceiling — it is a baseline. You can add personas locally via `/muse:build` in ~15 interactive questions, or pull new personas from the community via a future persona catalog (roadmap item — see § Roadmap).

Shipped artifacts:

### 🎭 Starter pack — 24 cognitive personas
...
You don't need to memorize 24 personas (or 80 someday) to use muse. Three entry points, from cold-start to expert:
...
MUSE TRIAGE — scoring 24 personas against your question
...
## Commands reference (35+ total — v2.28.0-alpha)
...
### 🎭 Persona sessions — the starter pack (24)
...
| Command | Attrs | What it does |
|---|---|---|
| `/muse:benchmark [--baseline \| --diff \| --quick \| --persona=<id>]` | 💾 | Measure persona distinctiveness + voice discipline + schema compliance + mode fit. 4 categories, 11 measures. Grade A maintained across all 24 personas. |
...
8. **Extensible by design.** The starter pack is 24 personas (growing). The runtime supports arbitrary local personas via `/muse:build`. A future persona catalog (§ Roadmap) will let users browse and clone individual personas without cloning the whole framework.
...
- `/muse:benchmark` measures distinctiveness empirically, not just by taste. Grade A across all 24 personas.
...
**Shipping today** (v2.28.0-alpha):
- 24 starter-pack personas across 8+ categories (philosophy, first-principles, design, systems, strategy, execution, investing, management/culture, AI-industry)
- 35+ slash commands (13 meta + 24 persona), 5 adaptive session modes (QUICK, STANDARD, DEEP, CRITIC, EXPLORE)
...
v2.28 is the mature foundation; v3.0 is when the catalog becomes real.
