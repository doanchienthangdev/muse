...
/muse:who should I rewrite this service in Rust?
  → Scores all 27 personas. Top 3: Elon Musk 72/80 (the algorithm +
...
You don't need to memorize 27 personas (or 80 someday) to use muse. Three entry points, from cold-start to expert:
...
MUSE TRIAGE — scoring 27 personas against your question
...
## Commands reference (35+ total — v2.31.0-alpha)
...
| Command | Attrs | What it does |
|---|---|---|
| `/muse:benchmark [--baseline \| --diff \| --quick \| --persona=<id>]` | 💾 | Measure persona distinctiveness + voice discipline + schema compliance + mode fit. 4 categories, 11 measures. Grade A maintained across all 27 personas. |
...
8. **Extensible by design.** The starter pack is 27 personas (growing). The runtime supports arbitrary local personas via `/muse:build`. A future persona catalog (§ Roadmap) will let users browse and clone individual personas without cloning the whole framework.
...
- `/muse:benchmark` measures distinctiveness empirically, not just by taste. Grade A across all 27 personas.
...
**Shipping today** (v2.31.0-alpha):
- 27 starter-pack personas across 8+ categories (philosophy, first-principles, design, systems, strategy, execution, investing, management/culture, AI-industry)
- 35+ slash commands (13 meta + 27 persona), 5 adaptive session modes (QUICK, STANDARD, DEEP, CRITIC, EXPLORE)
