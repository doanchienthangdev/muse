...
/muse:who should I rewrite this service in Rust?
  → Scores all 28 personas. Top 3: Elon Musk 72/80 (the algorithm +
...
13 meta/orchestration commands + 27 persona commands. All self-contained, all structured step-by-step with validation and persistence. See § Commands reference for full list with characteristics.
...
You don't need to memorize 28 personas (or 80 someday) to use muse. Three entry points, from cold-start to expert:
...
MUSE TRIAGE — scoring 28 personas against your question
...
## Commands reference (36+ total — v2.35.0-alpha)
...
| Command | Attrs | What it does |
|---|---|---|
| `/muse:benchmark [--baseline \| --diff \| --quick \| --persona=<id>]` | 💾 | Measure persona distinctiveness + voice discipline + schema compliance + mode fit. 4 categories, 11 measures. Grade A maintained across all 28 personas. |
...
8. **Extensible by design.** The starter pack is 28 personas (growing). The runtime supports arbitrary local personas via `/muse:build`. A future persona catalog (§ Roadmap) will let users browse and clone individual personas without cloning the whole framework.
...
- `/muse:benchmark` measures distinctiveness empirically, not just by taste. Grade A across all 28 personas.
