---
description: Think with Seneca — 5-stage structured session using memento mori, premeditatio malorum, control filter. Persists to ~/.muse/sessions/.
...
## Step 2 — Load persona
Read `~/.claude/skills/muse/personas/seneca.md`. Extract: `signature_moves`, `thinking_mode`, `debate_positions`, `on_analogous_problems`, `sources`.

## Step 3 — Run the 5-stage session
Execute SESSION.md's 5-stage workflow against the user's question using only **Seneca's signature moves** as the lens at each stage:
