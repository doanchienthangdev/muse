...
**v2.13 additions**: Synthesis Plan + Audit (Step 3.5 in `/muse:build`) as the producer-consumer contract between pipeline findings and drafter. Inline gates in Step 4 (per-move ghost-citation, incremental distinctiveness, balanced-provenance). Save-gate benchmark + balanced-provenance (Step 5.95b). Year-archive granularity heuristic + stratified sampling (Step 2B). Closes the meta-bug where the pipeline extracted correctly but the drafter under-used findings.

**v2.14 additions**: "Pipeline brains" — envelope persistence to `.archives/personas/<id>/_pipeline_output/` (last 5 per persona), LLM-judge semantic similarity (two-tier Jaccard + Agent dispatch for 0.3-0.9 borderline band, replaces Stage 4 Test 3 lexical-only exclusive filter), context-budget awareness per subagent (50%/80% thresholds), book-length adaptive PDF budget (20% coverage, floor 50, ceiling 200 pages), rejected-candidates surfaced to spec reviewer (Dimension 6 filter-quality audit), auto-retry on subagent timeout (one retry with simplified prompt).

**v2.15 additions**: "Living and fresh" — `corpus_fingerprint` frontmatter field (mtime + bucket-count snapshot, optional) for staleness detection. NEW skills: `/muse:refresh <id>` (incremental delta-mine for living figures) and `/muse:rebuild <id>` (safe full rebuild with A/B comparison + user choice). `/muse:update` C3/C6/C9/C10/C11 extended to route through the v2.14 research pipeline for primary-source-grounded fixes (previously inline inference only). Auto-rewrite on ghost-citation fail (both inline Step 4.3.1 and final Step 5.95): Agent subagent finds verified verbatim quotes in the source file to replace fabricated ones. Absolute verify-or-refuse guarantee preserved; usually saves the move without user intervention.
