# Getting Started with Muse (v2.3.2-alpha)

A 5-minute walkthrough to your first productive session, including the cold-start entry point (`/muse:who`) and the 18 slash commands available.
...
2. Copies all 18 slash commands to `~/.claude/commands/muse:*.md`
...
Shows all installed personas grouped by category (first-principles, systems, philosophy, design, etc.). Starter pack ships with 8 personas, but the list is dynamic — any persona you add locally via `/muse:build` appears automatically.
...
The starter pack is 8 personas. To add a 9th (say, Jane Jacobs on cities):
...
18 slash commands total. See the [README Commands reference](../README.md#commands-reference-18-total--v232-alpha) for the full table with attributes (interactive, persistent, multi-persona, requires-API, quick).
...
*v2.3.2-alpha · 18 slash commands · 8 starter-pack personas · extensible runtime*
